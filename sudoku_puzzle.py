...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Returns True or False, depending on whether it is clear that the
        current puzzle configuration can never be completed, through a sequence
        of extensions, to a solved state.
        ...
        for i in range(0, self._n ** 2):
            if self._symbols[i] == "*":
                symbol_all = self._symbol_set.copy() | {"*"}
                union_set = (self._row_set(i) | self._column_set(
                    i) | self._subsquare_set(i))
                if len(symbol_all) - len(union_set) == 0:
                    return True
...
if __name__ == "__main__":
    ...
    start = time()
    # sol = breadth_first_solve(s)  # new bfs
